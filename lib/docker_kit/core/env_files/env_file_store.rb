class DockerKit::Core::EnvFiles::EnvFileStore
  ...
  def add(env_file)
    ...
    if !env_file.is_a?(DockerKit::Core::EnvFiles::AbstractEnvFile)
      raise ArgumentError.new("should be an instance of DockerKit::Core::EnvFiles::AbstractEnvFile, got: #{env_file.inspect}")
    end
  ...
  def get_from_configuration(env_file_name)
    env_files = DockerKit.current_configuration.env_files
