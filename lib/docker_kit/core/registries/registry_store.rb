class DockerKit::Core::Registries::RegistryStore
  ...
  def add(registry)
    ...
    if !registry.is_a?(DockerKit::Core::Registries::AbstractRegistry)
      raise ArgumentError.new("should be an instance of DockerKit::Core::Registries::AbstractRegistry, got: #{registry.inspect}")
    end
  ...
  def get_from_configuration(registry_name)
    registries = DockerKit.current_configuration.registries
    registries[registry_name]
  end

  def default_registry
    @default_registry ||= DockerKit::Core::Registries::Registry.new(:default)
  end
