class DockerKit::Actions::ConfigurationLoader
  ...
  Contract Hash => Any
  def call(options)
    root_path   = options[:path] || File.join(Dir.pwd, configs.docker_kit_dirname)
    ...
    logger.info "Launching docker_kit with:"
    ...
    unless File.exists?(root_path)
      ui.print_warning "WARNING", "DockerKit root path #{root_path} doesn't exist. You may want to pass it --path parameter."
    end
    ...
    ui.create_task("Updating artifacts") do |task|
      artifacts = DockerKit.current_configuration.artifacts.values
  ...
  def load_configurations(configurations_path, configuration_name)
    ...
    if configuration_store.count > 1 && configuration_name.nil?
      raise DockerKit::Error, "Please set configuration name using -C option"
    end

    DockerKit.set_configuration_name(configuration_name)
  end
  ...
  rescue DockerKit::Shell::AbstractShell::DirNotFoundError
