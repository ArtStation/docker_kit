RSpec.describe DockerKit::Actions::ConfigurationLoader do
  subject{ DockerKit::Actions::ConfigurationLoader.new }

  it "loads configuration definitions based on configuration_path" do
    expect(subject).to receive(:load_configurations).with("/opt/docker_kit/configs", nil)
    subject.call(configurations_path: "/opt/docker_kit/configs")
  end

  it "loads configuration definitions based on default path" do
    expect(subject).to receive(:load_configurations).with("/opt/docker_kit/configurations", nil)
    subject.call(path: "/opt/docker_kit")
  end
  ...
  context "when one configuration exists" do
    ...
    it "uses first found configuration" do
      ...
      expect(DockerKit.current_configuration.name).to eq(:some_configuration)
    end
  end
  ...
  context "when multiple configuration exists" do
    ...
    it "raises error if no configuration is selected" do
      ...
      expect{ subject.call({}) }.to raise_error(DockerKit::Error)
    end
  end
end