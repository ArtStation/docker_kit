RSpec.describe DockerKit::Shell::LocalShell do
  subject{ DockerKit::Shell::LocalShell.new }
  ...
  context "#exec!" do
    ...
    it "raises error if exit status is not 0" do
      ...
      }.to raise_error(DockerKit::Shell::LocalShell::ShellError) 
    end
  end
  ...
  context "#recursive_list_files" do
    ...
    it "raises not found error if directory not found" do
      expect(subject).to receive(:exec!).and_raise(DockerKit::Shell::AbstractShell::ShellError.new("No such file or directory"))

      expect{ subject.recursive_list_files(test_dir) }.to raise_error(DockerKit::Shell::AbstractShell::DirNotFoundError)
    end

    it "reraises error if different error happens" do
      expect(subject).to receive(:exec!).and_raise(DockerKit::Shell::AbstractShell::ShellError.new("Random error"))

      expect{ subject.recursive_list_files(test_dir) }.to raise_error(DockerKit::Shell::AbstractShell::ShellError)
    end
  end
end