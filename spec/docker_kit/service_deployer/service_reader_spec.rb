RSpec.describe DockerKit::ServiceDeployer::ServiceReader do
  subject{ DockerKit::ServiceDeployer::ServiceReader.new }

  let(:artifact) { DockerKit::Core::Artifacts::Local.new(:templates).setup(File.join(FIXTURES_PATH, "templates")) }
  let(:template) { DockerKit::Core::Templates::ArtifactFile.new(:service_template, artifact_name: :templates, file_path: "service.yml") }
