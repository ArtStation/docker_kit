...
RSpec.describe DockerKit::Core::EnvFiles::EnvFileStore do
  subject{ DockerKit::Core::EnvFiles::EnvFileStore.new }

  context "#get_global" do
    it "returns global env_file" do
      env_file = DockerKit::Core::EnvFiles::ArtifactFile.new(:default, artifact_name: :env_files, file_path: "path/to.file")
    ...
    it "raises error if env_file is not found" do
      expect{ subject.get_global(:default) }.to raise_error(DockerKit::Core::EnvFiles::EnvFileStore::NotFoundError)
    end
  end

  context "#get_from_configuration" do
    it "returns configuration env_file" do
      env_file = DockerKit::Core::EnvFiles::ArtifactFile.new(:production_default, artifact_name: :env_files, file_path: "path/to.file")
      subject.add(env_file)
  ...
  context "#add" do
    it "doesn't allow adding env_file twice" do
      ...
      expect{ subject.add(env_file) }.to raise_error(DockerKit::Core::EnvFiles::EnvFileStore::AlreadyAddedError)
    end
