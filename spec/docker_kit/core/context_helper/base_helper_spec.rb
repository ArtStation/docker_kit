RSpec.describe DockerKit::Core::ContextHelper::BaseHelper do
  subject{ DockerKit::Core::ContextHelper::BaseHelper.new(
    image_store: test_helper.image_store,
    artifact_store: DockerKit::Container['core.artifact_store'],
  ...
  context "artifact_path" do
    it "returns artifacts cloned path" do
      ...
      expect(subject.artifact_path(:example_repo)).to eq("/tmp/docker_kit/artifacts/example_repo")
    end

    it "returns artifacts path to specific file if needed" do
      test_helper.add_artifact(:example_repo, "git@github.com/myapp.git")

      expect(subject.artifact_path(:example_repo, "test.txt")).to eq("/tmp/docker_kit/artifacts/example_repo/test.txt")
    end
  end
