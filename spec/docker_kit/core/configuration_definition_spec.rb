...
RSpec.describe DockerKit::Core::ConfigurationDefinition do
  subject{ DockerKit::Core::ConfigurationDefinition.new(:production) }

  context "initialize" do
    it "can initialize configuration with symbol name" do
      definition = DockerKit::Core::ConfigurationDefinition.new(:production)
  ...
  context "artifact" do
    ...
    it "doesn't allow duplicates" do
      ...
      expect{
        subject.use_artifact(:main_production_repo, as: :main_repo)
      }.to raise_error(DockerKit::Core::ConfigurationDefinition::ResourceAlreadyAdded)
    end
  end
