...
RSpec.describe DockerKit::Core::ConfigurationStore do
  subject{ DockerKit::Core::ConfigurationStore.new }
  ...
  context "#define" do
    it "defines an configuration and returns ConfigurationDefinition" do
      ...
      expect(definition).to be_a(DockerKit::Core::ConfigurationDefinition)
    end
  end
  ...
  context "#get_definition" do
    ...
    it "raises NotFound error if image is not found" do
      expect{ subject.get_definition(:production) }.to raise_error(DockerKit::Core::ConfigurationStore::NotFoundError)
    end
  end
  ...
  context "#add_definition" do
    it "adds ConfigurationDefinition object to the store" do
      subject.add_definition(test_definition)
      expect(subject.get_definition(:production)).to be_a(DockerKit::Core::ConfigurationDefinition)
    end

    it "doesn't allow adding a definition twice" do
      subject.add_definition(test_definition)
      expect{ subject.add_definition(test_definition) }.to raise_error(DockerKit::Core::ConfigurationStore::AlreadyAddedError)
    end
  end
  ...
  context "#get_configuration" do
    it "returns a configuration based on definition" do
      ...
      expect(configuration).to be_a(DockerKit::Core::Configuration)
