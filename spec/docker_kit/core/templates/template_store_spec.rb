...
RSpec.describe DockerKit::Core::Templates::TemplateStore do
  subject{ DockerKit::Core::Templates::TemplateStore.new }

  context "#get_global" do
    it "returns global template" do
      template = DockerKit::Core::Templates::ArtifactFile.new(:default, artifact_name: :env_files, file_path: "path/to.file")
    ...
    it "raises error if template is not found" do
      expect{ subject.get_global(:default) }.to raise_error(DockerKit::Core::Templates::TemplateStore::NotFoundError)
    end
  end

  context "#get_from_configuration" do
    it "returns configuration template" do
      template = DockerKit::Core::Templates::ArtifactFile.new(:production_default, artifact_name: :env_files, file_path: "path/to.file")
      subject.add(template)
  ...
  context "#add" do
    it "doesn't allow adding template twice" do
      ...
      expect{ subject.add(template) }.to raise_error(DockerKit::Core::Templates::TemplateStore::AlreadyAddedError)
    end
