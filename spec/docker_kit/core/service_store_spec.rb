...
RSpec.describe DockerKit::Core::ServiceStore do
  subject{ DockerKit::Core::ServiceStore.new }
  ...
  context "#define" do
    it "defines an service and returns ServiceDefinition" do
      ...
      expect(definition).to be_a(DockerKit::Core::ServiceDefinition)
    end
  ...
  context "#get_definition" do
    ...
    it "raises NotFound error if service is not found" do
      expect{ subject.get_definition(:example) }.to raise_error(DockerKit::Core::ServiceStore::NotFoundError)
    end
  end
  ...
  context "#add_definition" do
    it "adds ServiceDefinition object to the store" do
      subject.add_definition(test_definition)
      expect(subject.get_definition(:example)).to be_a(DockerKit::Core::ServiceDefinition)
    end

    it "doesn't allow adding a definition twice" do
      subject.add_definition(test_definition)
      expect{ subject.add_definition(test_definition) }.to raise_error(DockerKit::Core::ServiceStore::AlreadyAddedError)
    end
  end
