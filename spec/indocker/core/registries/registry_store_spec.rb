...
RSpec.describe Indocker::Core::Registries::RegistryStore do
  ...
  context "#get_global" do
    ...
    it "raises error if registry is not found" do
      expect{ subject.get_global(:default) }.to raise_error(Indocker::Core::Registries::RegistryStore::NotFoundError)
    end
  end
  ...
  context "#add" do
    it "doesn't allow adding registry twice" do
      ...
      expect{ subject.add(registry) }.to raise_error(Indocker::Core::Registries::RegistryStore::AlreadyAddedError)
    end
