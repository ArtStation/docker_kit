...
RSpec.describe Indocker::Core::Artifacts::ArtifactStore do
  subject{ Indocker::Core::Artifacts::ArtifactStore.new }

  context "#get_global" do
    it "returns global artifact" do
      artifact = Indocker::Core::Artifacts::Git.new(:default)
    ...
    it "raises error if artifact is not found" do
      expect{ subject.get_global(:default) }.to raise_error(Indocker::Core::Artifacts::ArtifactStore::NotFoundError)
    end
  end

  context "#get_from_configuration" do
    it "returns configuration artifact" do
      artifact = Indocker::Core::Artifacts::Git.new(:production_default)
      subject.add(artifact)
  ...
  context "#add" do
    it "doesn't allow adding artifact twice" do
      ...
      expect{ subject.add(artifact) }.to raise_error(Indocker::Core::Artifacts::ArtifactStore::AlreadyAddedError)
    end
